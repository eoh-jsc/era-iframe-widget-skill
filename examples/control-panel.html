    ...
    <div class="dashboard">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <span class="card-title" id="tempLabel">Temperature</span>
        </div>
        <div class="value-lg" id="tempValue">--</div>
      ...
      <div class="card">
        <div class="card-header">
          ...
          <span class="card-title" id="humidLabel">Humidity</span>
        </div>
        <div class="mini-gauge">
          ...
          <div class="center" id="humidValue">--%</div>
        </div>
      </div>
      ...
      <div class="card wide">
        ...
        <div id="controlsContainer">
          <div class="control-row">
            <span class="control-label" id="control1Label">Device 1</span>
            <label class="switch">
              <input type="checkbox" id="control1" />
              <span class="slider"></span>
            </label>
          </div>
          <div class="control-row">
            <span class="control-label" id="control2Label">Device 2</span>
            <label class="switch">
    ...
    <script>
      ...
      eraWidget.init({
        ...
        onConfiguration: (configuration) => {
          ...
          // Use first 2 configs for displays, next 2 for controls
          if (configs[0])
            tempLabel.textContent = configs[0].name || 'Temperature';
          if (configs[1])
            humidLabel.textContent = configs[1].name || 'Humidity';
          if (configs[2])
            control1Label.textContent = configs[2].name || 'Device 1';
          if (configs[3])
            control2Label.textContent = configs[3].name || 'Device 2';
        },

        onValues: (values) => {
          // Temperature
          if (configs[0]) {
            const temp = parseFloat(values[configs[0].id].value).toFixed(1);
          ...
          if (configs[1]) {
            ...
            const offset =
              circumference - (Math.min(humid, 100) / 100) * circumference;
      ...
      control1.addEventListener('click', () => {
        ...
        if (actions[actionIdx]) {
          eraWidget.triggerAction(actions[actionIdx].action, null);
        }
      });
